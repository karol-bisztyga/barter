...
async function createBucket(bucketName) {
  try {
    const response = await b2.createBucket({
      bucketName,
      bucketType: 'allPrivate', // You can also use 'allPublic'
    });
...
const getBucketIdByName = async (bucketName) => {
  ...
  if (!bucketId) {
    throw new Error('bucket id not foudn for bucket name', bucketName);
  }
  return bucketId.id;
};
...
module.exports = { uploadRandomImageB2, authenticate };
